        ...
        paste! {
                ...
                C: Fn($date_type) + 'static,
            {
                let text = match format_string.clone() {
                    Some(format_string) => create_memo(move |_| value.get().format(&format_string).to_string()),
                    None => create_memo(move |_| value.get().to_string()),
                };

                if editable {
                    let td_ref = create_node_ref::<Td>();
    ...
    C: Fn(DateTime<Utc>) + 'static,
{
    let text = match format_string.clone() {
        Some(format_string) => create_memo(move |_| value.get().format(&format_string).to_string()),
        None => create_memo(move |_| value.get().to_rfc3339()),
    };
